...
#@jit(nopython=True)
def compute_rho(rho, ions, electrons):
    rho[0:Nx] = ions.den[0:Nx] -  electrons.den[0:Nx]
...
for ts in range(0, T+1):
    ...
    compute_rho(rho, ions, electrons)
    ...
    #ax.clear()
    if (ts % interval == 0):
        print("TS: {}\t nI: {} \t nE: {} \t ke_e: {:.4f} \t ke_i: {:.4f}  \t total energy: {:.4f}  \t max_phi: {:.4f} ".format(
        ts, len(ions.part_list), len(electrons.part_list), kee, kei, kee + kei + pe, max_phi))
